...
#   ) %>%
#   mutate(exp_phase = if_else(as.numeric(n_sesion) >= 13, "experimental", "basal"),
...
df_summ <- df %>% 
  ...
  mutate(
    evento_delta = evento_summ - evento_basal,
    actividad_delta = actividad_summ - actividad_basal
  )
...
df_summ %>% 
  ...
  summarise(m = mean(actividad_delta))
  
  
df_summ %>% 
  filter(exp_phase == "experimental", exp_group == "experimental") %>% 
  ggplot(aes(tipo_recompensa, actividad_delta)) +
  geom_boxplot()
